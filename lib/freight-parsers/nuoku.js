...
function isNuokuSupportedPriceType(typeLabel) {
  return typeLabel.includes('含税');
}

function isNuokuUnsupportedPriceType(typeLabel) {
  return typeLabel.includes('自税');
}
...
function buildNuokuSections(headerColumns) {
  ...
  while (index < headerColumns.length) {
    if (!isNuokuSupportedPriceType(headerColumns[index].typeLabel)) {
      index += 1;
      continue;
    }
    ...
    while (index < headerColumns.length && isNuokuSupportedPriceType(headerColumns[index].typeLabel)) {
    ...
    while (index < headerColumns.length) {
      ...
      if (isNuokuSupportedPriceType(column.typeLabel)) {
        break;
      }

      if (isNuokuUnsupportedPriceType(column.typeLabel)) {
        while (index < headerColumns.length && isNuokuUnsupportedPriceType(headerColumns[index].typeLabel)) {
          index += 1;
        }
        break;
      }
...
function collectNuokuRecordsFromSheet(sheetName, sheet) {
  ...
  const sectionCandidates = sections
    .map((section) => ({
      section,
      yiwuColumn: pickNuokuPriceColumn(section, scoreNuokuYiwuRegion),
      shenzhenColumn: pickNuokuPriceColumn(section, scoreNuokuShenzhenRegion)
    }))
    .filter((item) => item.yiwuColumn || item.shenzhenColumn);
  ...
  for (let rowNumber = 7; rowNumber <= range.e.r + 1; rowNumber += 1) {
    ...
    const sectionMatch = sectionCandidates
      .map(({ section, yiwuColumn, shenzhenColumn }) => ({
        section,
        yiwuColumn,
        shenzhenColumn,
        yiwuPrice: yiwuColumn
          ? toNumber(sheetValueByColumnNumber(sheet, yiwuColumn.columnNumber, rowNumber))
          : null,
        shenzhenPrice: shenzhenColumn
          ? toNumber(sheetValueByColumnNumber(sheet, shenzhenColumn.columnNumber, rowNumber))
          : null
      }))
      .find((item) => item.yiwuPrice !== null || item.shenzhenPrice !== null);
    ...
    records.push({
      ...
      rawWarehouseCode,
      warehouseCode,
      yiwuPackageTaxPrice: sectionMatch.yiwuPrice,
      shenzhenPackageTaxPrice: sectionMatch.shenzhenPrice,
      yiwuOriginLabel: sectionMatch.yiwuColumn ? sectionMatch.yiwuColumn.regionLabel : '',
      shenzhenOriginLabel: sectionMatch.shenzhenColumn ? sectionMatch.shenzhenColumn.regionLabel : '',
      taxStartStandard: sectionMatch.yiwuColumn?.startStandard || sectionMatch.shenzhenColumn?.startStandard || '',
      referenceAging: sectionMatch.section.timeColumnNumber
        ? toText(sheetValueByColumnNumber(sheet, sectionMatch.section.timeColumnNumber, rowNumber))
        : '',
      compensationAging: sectionMatch.section.compensationColumnNumber
        ? toText(sheetValueByColumnNumber(sheet, sectionMatch.section.compensationColumnNumber, rowNumber))
        : ''
    });
  }
